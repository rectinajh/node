...
var outputDirectory = flag.String("output", "gen_db.go", "Name of output source directory with binary data")
...
func main() {
	...
	parts := splitToFixedLengthSlices(encodedData, 64*1024) // 64KB parts
	var dbParts []dbPart
	...
	exitOnError(err)
	err = tmpl.Execute(idxFile, dbParts)
	exitOnError(err)
}
...
func newDbPart(id int, binaryData []byte) dbPart {
	var lines []string
	for _, binaryPart := range splitToFixedLengthSlices(binaryData, 120) {
...
const DbData = ""{{range .}} + dbDataPart{{.ID}}{{end}}
`
